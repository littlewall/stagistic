...
export const getNodeAttrs = (node: ScriptNode): Record<string, unknown> | undefined => {
    return node.attrs && typeof node.attrs === 'object' ? node.attrs : undefined;
};
...
export const getScriptBlockNodeType = (node: ScriptNode, fallback: ScriptBlockNodeType = DEFAULT_SCRIPT_BLOCK_NODE_TYPE): ScriptBlockNodeType => {
...
export const createScriptBlockNode = (rawBlockType: unknown = DEFAULT_SCRIPT_BLOCK_NODE_TYPE, id?: string): ScriptNode => {
...
export const createEmptyScriptDocument = (blockId?: string, settings?: EditorSettingsOverride): ScriptDocument => {
...
export const createActlessScriptDocument = (blockId?: string, settings?: EditorSettingsOverride): ScriptDocument => ({
    type: 'doc',
    attrs: settings ? {settings} : undefined,
    content: [createScriptBlockNode('scene', blockId)],
});

export const createDefaultScriptDocument = (blockId?: string, settings?: EditorSettingsOverride): ScriptDocument => {

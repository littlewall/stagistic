...
Allow uploading PDF files to music in the Attribute Manager. The app is
...
- N PDFs per music (list of attachments).
...
types, attachments on entities other than music (schema leaves room; no UI).
...
**`script_music_attachments`** — join only:
- `music_id` (text, FK → script_music.id, ON DELETE CASCADE)
- `attachment_id` (text, FK → script_attachments.id, ON DELETE CASCADE)
- `sort_order` (integer)
- `created_at` (bigint)
- Index on `music_id` and on `attachment_id`.

Migration is hand-written, then `pnpm --filter @stagistic/db db:compile-migrations`.

Queries: `insertAttachment`, `insertMusicAttachmentLink`, `listAttachmentsByMusic`,
`getAttachmentById`, `deleteMusicAttachmentLink`, `deleteAttachment`,
`countAttachmentLinks`.

### 3. Repository handlers — `createAttachmentHandlers`

Mirrors `music.ts` / `locations.ts` (`getDb`, `recordOutbox`, `syncDb`, plus
injected `fileStorage`).

- `listByMusic(musicId)` → attachment metadata rows (joined).
- `attachToMusic(scriptId, musicId, file)`:
  `storageKey = fileStorage.save(file)` → insert `script_attachments` → insert
  `script_music_attachments` → `recordOutbox('musicAttachment.attach')` → `syncDb()`.
- `removeFromMusic(scriptId, musicId, attachmentId)`: delete join row; if
  `countAttachmentLinks(attachmentId) === 0` → delete `script_attachments` row +
  `fileStorage.delete(storageKey)` (orphan GC) → `recordOutbox('musicAttachment.detach')`
  → `syncDb()`.
...
The Music panel already exists as a list; its **detail** is currently the
placeholder `"Music details are coming soon."` (generic `AttributeManagerListPanel`
...
- **`AttributeManagerListPanel`**: add optional `renderDetail(selectedItem)`
  render-prop. In the provider, pass it for the Music panel to replace
  `detailPlaceholder`. Music list logic (`buildAttributeManagerMusicItems`,
  assigned/unassigned, numbering) is untouched.
- **Music detail content**: an "Attachments" section — list of PDFs (filename,
...
- **`MusicAttachmentPreviewModal`**: reuses the `pdfjs-dist` canvas render from
...
- **`useMusicAttachmentsState`** hook: mirrors `useScriptPlacesState`, wired in the
...
- DB queries + handlers: PGlite node tests (project convention) — attach, list,
  remove, orphan GC, cascade on music delete. Use an in-memory `FileStorage`.
- UI: browser tests (`test:browser`) for the music detail attachment list, upload
